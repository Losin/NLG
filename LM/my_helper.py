...
class MySampleHelper(helper.Helper):
    """A helper for first generate given prefix, then generate the reminder by sampling.
    """
    def __init__(self, inputs, sequence_length, end_token, embedding, seed=None, time_major=False, name=None):
    ...
    def sample(self, time, outputs, name=None, **unused_kwargs):
        ...
        sample_ids = control_flow_ops.cond(
              ...
              lambda: self._input_tas.read(time+1))
    ...
    def next_inputs(self, time, outputs, state, sample_ids, name=None):
        with ops.name_scope(name, "myHelperNextInputs",
                            [time, outputs, state]):
            next_time = time + 1
            finished = math_ops.equal(sample_ids, self._end_token) #array_ops.tile([False], [self._batch_size])#
...
class MyGreedyHelper(helper.Helper):
    """A helper for first generate given prefix, then generate the reminder by sampling.
    """
    def __init__(self, inputs, sequence_length, end_token, embedding, seed=None, time_major=False, name=None):
    ...
    def sample(self, time, outputs, name=None, **unused_kwargs):
        ...
        sample_ids = control_flow_ops.cond(
              all_prefixed, lambda: math_ops.cast(math_ops.argmax(outputs, axis=-1), dtypes.int32),
              lambda: self._input_tas.read(time+1))
